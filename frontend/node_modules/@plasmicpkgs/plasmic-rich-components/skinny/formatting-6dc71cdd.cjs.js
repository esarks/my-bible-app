...
function getFieldSubprops(opts) {
  return __spreadValues$1(__spreadValues$1({
    ...
    locale: {
      ...
      hidden: (ps, ctx, { item }) => !utils.isOneOf(item.dataType, NUMBER_TYPES) && !utils.isOneOf(item.dataType, DATETIME_TYPES)
    },
    notation: {
      ...
      hidden: (ps, ctx, { item }) => !utils.isOneOf(item.dataType, NUMBER_TYPES)
    },
    signDisplay: {
      type: "choice",
...
function maybeRenderString(record, cconfig) {
  return cconfig && !cconfig.isHidden ? utils.maybe(getFieldValue(record, cconfig), asString) : void 0;
}
...
function renderValue(record, cconfig) {
  ...
  if (cconfig.dataType === "auto") {
    ...
    if (utils.isOneOf(cconfig.dataType, NUMBER_TYPES) && typeof coerced === "number") {
...
function renderRelativeDate(value, cconfig) {
  ...
  if (utils.isOneOf(unit, UNITS_BY_MS)) {
...
function coerceValue(value, dataType) {
  ...
  try {
    if (utils.isOneOf(dataType, NUMBER_TYPES)) {
      if (typeof value === "number") {
        return value;
    ...
    } else if (utils.isOneOf(dataType, DATETIME_TYPES)) {
...
exports.buildFieldsPropType = buildFieldsPropType;
exports.deriveValueType = deriveValueType;
exports.getFieldAggregateValue = getFieldAggregateValue;
exports.getFieldSubprops = getFieldSubprops;
exports.maybeRenderString = maybeRenderString;
exports.multiRenderValue = multiRenderValue;
exports.renderValue = renderValue;
//# sourceMappingURL=formatting-6dc71cdd.cjs.js.map
