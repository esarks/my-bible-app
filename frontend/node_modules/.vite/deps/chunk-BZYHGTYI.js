...
var mutateKeys = (invalidateKey) => {
  if (__SWRConfig) {
    const { cache: cache2, mutate: mutate2 } = __SWRConfig;
    (invalidateKey != null ? [invalidateKey] : Array.from(cache2.keys())).forEach((key) => {
      mutate2(key);
    });
  }
};
...
function usePlasmicQueryData(key, fetcher) {
  const prepassCtx = import_react2.default.useContext(PrepassContext);
  ...
  const wrappedFetcher = import_react2.default.useMemo(
...
function useMutablePlasmicQueryData(key, fetcher, options) {
  ...
  const fetcherWrapper = import_react2.default.useCallback(
  ...
  const laggyDataRef = import_react2.default.useRef();
  const { isValidating, mutate: mutate2, data, error } = useSWR(
    key,
    fetcherWrapper,
    opts
  );
  ...
  return import_react2.default.useMemo(
    ...
    [isValidating, mutate2, data, error, isLoading]
  );
}
function PlasmicQueryDataProvider(props) {
  const { children, suspense, prefetchedCache } = props;
  const prepass = import_react2.default.useContext(PrepassContext);
  if (prepass) {
...
var PrepassContext = import_react2.default.createContext(false);
function PlasmicPrepassContext(props) {
  const { cache: cache2, children } = props;
  return import_react2.default.createElement(PrepassContext.Provider, { value: true }, import_react2.default.createElement(
    SWRConfig,
    {
      value: {
        provider: () => cache2,
...
function isPlasmicPrepass() {
  var _a2, _b, _c;
  return !!((_c = (_b = (_a2 = import_react2.default.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED) == null ? void 0 : _a2.ReactCurrentDispatcher) == null ? void 0 : _b.current) == null ? void 0 : _c.isPlasmicPrepass);
}
var HeadMetadataContext = import_react2.default.createContext({});
...
swr/dist/index.mjs:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-BZYHGTYI.js.map
